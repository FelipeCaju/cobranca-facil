...
-- Contas de recebimento: cada empresa pode conectar vários bancos/gateways,
-- mantendo exatamente uma conta padrão.
CREATE TABLE payment_accounts (
  ...
  created_at DATETIME(3) NOT NULL DEFAULT (CURRENT_TIMESTAMP(3)),
  updated_at DATETIME(3) NOT NULL DEFAULT (CURRENT_TIMESTAMP(3)) ON UPDATE CURRENT_TIMESTAMP(3),
  KEY idx_payment_accounts_company (company_id),
  CONSTRAINT fk_payment_accounts_company FOREIGN KEY (company_id) REFERENCES companies (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Histórico de cada retorno recebido dos provedores de pagamento.
...
CREATE TABLE message_delivery_log (
  ...
  CONSTRAINT fk_message_delivery_company FOREIGN KEY (company_id) REFERENCES companies (id) ON DELETE CASCADE,
  CONSTRAINT fk_message_delivery_installment FOREIGN KEY (installment_id) REFERENCES installments (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Lotes de importação e linhas rejeitadas, para implantação auditável por CSV/Excel.
CREATE TABLE import_jobs (
  id CHAR(36) NOT NULL PRIMARY KEY, company_id CHAR(36) NOT NULL,
  kind VARCHAR(32) NOT NULL, filename VARCHAR(255) NULL,
  status ENUM('preview','completed','failed') NOT NULL DEFAULT 'preview',
  total_rows INT NOT NULL DEFAULT 0, imported_rows INT NOT NULL DEFAULT 0, failed_rows INT NOT NULL DEFAULT 0,
  created_at DATETIME(3) NOT NULL DEFAULT (CURRENT_TIMESTAMP(3)),
  KEY idx_import_jobs_company_created (company_id, created_at),
